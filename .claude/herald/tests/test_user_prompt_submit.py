...
class TestUserPromptSubmitHook(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._patchers = [
            patch("herald.handlers.user_prompt_submit.PROMPT_LOG_PATH", self.prompt_log),
            patch("herald.handlers.user_prompt_submit.RATE_LIMIT_PATH", self.rate_log),
        ]
    ...
    def _dispatch(self, payload: dict, *, time_value: float | None = None):
        if time_value is None:
            return dispatch(constants.USER_PROMPT_SUBMIT, payload=payload)
        with patch("herald.handlers.user_prompt_submit.time.time", return_value=time_value):
            return dispatch(constants.USER_PROMPT_SUBMIT, payload=payload)
